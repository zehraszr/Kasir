...
class MainModel extends CI_Model
{
    ...
    public function cekBarangByNama($namaBarang){
        //SELECT count(kdBarang) as jml FROM barang WHERE namaBarang='GUCCI'
        $this->db->select('count(kdBarang) as jml');
        $this->db->where('namaBarang',$namaBarang);
        $sql=$this->db->get('barang');
        return $sql->row_array();
    }

     public function cekKategoriByNama($namaKategori){
        //SELECT count(kdBarang) as jml FROM barang WHERE namaBarang='GUCCI'
        $this->db->select('count(idKategori) as jml');
